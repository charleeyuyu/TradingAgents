...
1. **禁止出现未经解释的术语。** 不许直接写 MA20、均线多头排列、乖离率、RSI、MACD、支撑位、压力位、量能、筹码。
   ...
   **但翻译成大白话不等于可以说错。** 尤其注意下面这几个，必须按左边的说法写：
   - 均线（MA50 / MA200 等）= **"过去 50 天（或 200 天）的平均股价"**。
     **绝对不许说成"大家的平均买入成本"、"大家平均花多少钱买的"、"股民的持仓成本"。**
     均线是过去一段时间收盘价的平均值，跟任何人实际花了多少钱买入毫无关系。
     说成"成本"会让读者误以为"股价低于均线=大家都在亏钱"，这个推论是错的。
   - 支撑位 = "股价前几次跌到这个位置附近就停住并回升了"
   - 压力位 = "股价前几次涨到这个位置附近就上不去了"
   - 成交量 = "买卖的活跃程度"
...
### 🟢/🔴/🟡 公司名（代码）
（2-4句人话：结论是什么、为什么、有多大把握）

**参考价位**：（原文给了就列出来并逐个解释；没给就写"这次分析没有给出具体价位"）
